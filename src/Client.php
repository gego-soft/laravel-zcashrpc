...
class Client
{
    ...
    public function __construct($config = [])
    {
        ...
        $handlerStack->push(
            Middleware::mapResponse(function (ResponseInterface $response) {
                return ZcashdResponse::createFrom($response);
            }),
            'json_response'
        );
    ...
    public function request($method, $params = [])
    {
        try {
            ...
            if ($response->hasError()) {
                ...
                throw new Exceptions\ZcashdException($response->error());
            }
            ...
            if (
                ...
                throw new Exceptions\ZcashdException($exception->getResponse()->error());
            }
        ...
        } catch (Exceptions\ZcashdException $exception) {
            throw $exception;
        }
    }

    /**
     * Makes async request to zcash Core.
     *
     * @param string        $method
     * @param mixed         $params
        ...
        callable $onRejected = null)
    {
        ...
        $promise->then(
            function (ResponseInterface $response) use ($onFullfiled) {
                $error = null;
                if ($response->hasError()) {
                    $error = new Exceptions\ZcashdException($response->error());
                }
            ...
            function (RequestException $exception) use ($onRejected) {
                if (
                    ...
                    $exception = new Exceptions\ZcashdException(
    ...
    /**
     * Converts amount from satoshi to zcash.
     *
     * @param int $amount
     *
     * @return float
    ...
    /**
     * Converts amount from zcash to satoshi.
     *
     * @param float $amount
     *
     * @return int
